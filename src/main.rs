...
fn get_index() -> &'static Index {
    unsafe { GLOBAL_INDEX.as_ref().unwrap() }
}

fn prepare_server(addr: std::net::SocketAddr) -> Box<Future<Item = (), Error = io::Error> + Send> {
    ...
    let server = tcp.incoming().for_each(move |socket| {
        ...
        let joke_iter = idx.lines.iter().skip(from).take(to - from);

        let stream = iter_ok(joke_iter);
        let sink = LineSink::new(socket);
        let write_future = stream
            .forward(sink)
...
fn create_index<P: AsRef<Path>>(f: P) -> Result<(), std::io::Error> {
    ...
    unsafe {
        GLOBAL_INDEX = Some(Index { index: idx, lines });
    }
    Ok(())
}
